...
import {
  ...
  SkillGapResult,
  TalentMarketplaceItem,
  TalentMarketplaceQuery,
  CareerResumeBuilderResponse,
  InterviewPracticeResult,
  PortfolioResult,
  AIResponse,
  CareerRoadmapResult,
  LinkedInImportResult,
  CareerProfile as CareerProfileModelType,
} from "../types/careerAccelerator.types.js";

const sampleMarketplace: TalentMarketplaceItem[] = [
  {
    id: "mp-001",
    ...
    summary:
      "An entry-level design role for graduates who want to build product experiences for education platforms.",
    postedAt: "2026-06-30",
  },
  {
    id: "mp-002",
    ...
    summary:
      "Help grow a student learning marketplace with content, campaigns, and audience optimization.",
    postedAt: "2026-07-01",
  },
  {
    id: "mp-003",
    ...
    summary:
      "Build and maintain web applications for learning communities and portfolio sites.",
    postedAt: "2026-07-02",
  },
];
...
const sampleJobs = [
  {
    ...
    summary:
      "Work on learning dashboard interfaces, build responsive pages, and help improve user retention.",
    postedAt: "2026-07-01",
  },
  {
    id: "job-002",
    ...
    summary:
      "Translate student learning data into actionable insights to help improve course performance.",
    postedAt: "2026-07-02",
  },
  {
    id: "job-003",
    ...
    summary:
      "Support course creation, write learner-focused content, and help structure assessment journeys.",
    postedAt: "2026-07-03",
  },
];
...
export const generateCoverLetter = async (
  data: CareerCoverLetterRequest,
): Promise<AIResponse> => {
  const prompt = `Write a personalized cover letter for ${data.name} applying to ${data.company} for a ${data.targetRole} role${
    data.recipientName ? ` addressed to ${data.recipientName}` : ""
  }. Use the applicant's experience, skills, education, accomplishments, and motivation.
...
export const searchAyonaireJobs = async (
  query: TalentMarketplaceQuery,
): Promise<TalentMarketplaceItem[]> => {
...
export const generatePortfolio = async (
  data: CareerPortfolioRequest,
): Promise<PortfolioResult> => {
  const prompt = `Create an AI portfolio summary and project showcase for ${data.name}, a ${data.headline}. Target role: ${data.targetRole || "N/A"} in ${
    data.industry || "a relevant industry"
  }.
...
export const buildResume = async (
  data: CareerResumeBuilderRequest,
): Promise<CareerResumeBuilderResponse> => {
    ...
    experience: [
      {
        ...
        dates: "2025 - Present",
        description:
          "Collaborated with peers to deliver learning solutions, build technical projects, and support team goals.",
      },
    ],
    education: [
      {
        ...
        dates: "2023 - 2026",
        details:
          "Studied modern technologies, digital product development, and career-ready project delivery.",
      },
    ],
...
Provide a skill gap summary, missing skills, recommended resources, and a suggested timeline.`;
  ...
  const missingSkills = data.currentSkills.length
    ? ["Communication", "Domain knowledge", "Problem solving"].filter(
  ...
  return {
    ...
    suggestedTimeline:
      "3-6 months with consistent learning, hands-on projects, and mentorship.",
  };
};

export const getTalentMarketplace = async (
  query?: TalentMarketplaceQuery,
): Promise<TalentMarketplaceItem[]> => {
  if (!query || (!query.keywords && !query.location && !query.skills && !query.role)) {
...
export const saveFreelanceProfile = async (
  userId: string,
  data: FreelanceProfileRequest,
): Promise<CareerProfileModelType> => {
...
Learning preferences: ${data.learningPreferences || "N/A"}`;
  ...
  return {
    ...
    timeline:
      data.timeline || "6 months with weekly goals, milestone reviews, and regular practice.",
  };
};

export const generateAIInterview = async (
  data: CareerInterviewRequest,
): Promise<InterviewPracticeResult> => {
  const prompt = `Create a list of interview practice questions and suggested answers for a ${
    data.targetRole
  } at ${data.experienceLevel || "early career"} level. Focus on ${
    data.focusAreas?.join(", ") || "core strengths and problem solving"
  }.`;
