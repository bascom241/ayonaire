const authEnvelope = (data: Record<string, unknown>) => ({
  type: "object",
  properties: {
...
export default {
  ...
  "/api/v1/auth/login": {
    post: {
      ...
      description:
        "Authenticates a user, returns a short-lived access token and a persisted refresh token session. Suspended and inactive accounts cannot login.",
      security: [],
      requestBody: {
        required: true,
        content: {
  ...
  "/api/v1/auth/refresh-token": {
    post: {
      ...
      description:
        "Validates the refresh token, revokes it, creates a new refresh session, and returns a new access/refresh token pair. Reuse of a revoked refresh token revokes all active sessions for that user.",
      security: [],
      requestBody: {
        required: true,
        content: {
  ...
  "/api/v1/auth/logout": {
    post: {
      ...
      summary: "Logout user",
      description:
        "Revokes the provided refresh token. Set allDevices to true to revoke all refresh sessions for the same user. Access tokens remain valid until expiry, so frontend should clear stored tokens immediately.",
      security: [],
      requestBody: {
        required: true,
        content: {
  ...
  "/api/v1/auth/user/{id}/deactivate-user": {
    put: {
      ...
      description:
        "Deactivates a user account. Admin only. The legacy misspelled route /api/v1/auth/user/{id}/deativate-user is still available for backward compatibility.",
      security: [{ bearerAuth: [] }],
      parameters: [userIdParam],
      responses: {
        200: {
