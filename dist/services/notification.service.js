...
import { NotificationRecipientType, NotificationStatus, } from "../types/notification.types.js";
...
const resolveRecipientEmails = async (recipientType, recipients, courseId, cohortId) => {
    if (cohortId) {
        const cohort = await cohortModel.findById(cohortId).populate("students", "email");
        return cohort?.students?.map((s) => s.email) || [];
    }
    if (courseId) {
        const course = await courseModel.findById(courseId).populate("students", "email");
        return course?.students?.map((s) => s.email) || [];
    }
...
export const sendNotificationNow = async (notificationId) => {
    ...
    if (notification.channel === "email" ||
        ...
        const emails = await resolveRecipientEmails(notification.recipientType, notification.recipients?.map((r) => r.toString()), notification.course?.toString(), notification.cohort?.toString());
        const result = await sendBulkNotificationEmail(emails, notification.name, notification.message);
