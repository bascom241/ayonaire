...
export const connectGateway = async (userId, data) => {
    ...
    const gateway = await paymentGatewayModel.findOneAndUpdate({ name: data.name }, {
        name: data.name,
        isConnected: true,
        mode: data.mode || "test",
...
export const updateGateway = async (name, data) => {
...
export const disconnectGateway = async (name) => {
    const gateway = await paymentGatewayModel.findOne({ name });
    if (!gateway) {
        throw new AppError("gateway not connected", 404);
    }
...
export const setPrimaryGateway = async (name) => {
    const gateway = await paymentGatewayModel.findOne({ name });
...
/// ---------- Pricing Plans ---------- ///
export const listPricingPlans = async (query) => {
    const filter = {};
    if (query.course)
...
export const createPricingPlan = async (userId, data) => {
    validateRequestBodyWithValues(data, [
        "course",
        "price",
    ]);
...
export const updatePricingPlan = async (planId, data) => {
    ...
    if (data.accessType !== undefined)
        plan.accessType = data.accessType;
