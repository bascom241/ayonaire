...
export const refreshAuthToken = async (data, ip, userAgent) => {
    ...
    if (storedToken.revokedAt) {
        await refreshTokenModel.updateMany({ user: storedToken.user, revokedAt: { $exists: false } }, { revokedAt: new Date() });
...
export const logoutUser = async (data, userId) => {
    ...
    if (data.allDevices) {
        ...
        await refreshTokenModel.updateMany({ user: sessionUserId, revokedAt: { $exists: false } }, { revokedAt: new Date() });
