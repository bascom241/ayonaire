...
export const inviteTeamMember = async (email, role = "admin") => {
    if (!email) {
...
export const suspendTeamMember = async (id) => {
    const user = await userModel.findByIdAndUpdate(id, { status: UserStatus.SUSPENDED }, { new: true });
    if (!user) {
        throw new AppError("team member not found", 404);
    }
    return user;
};
export const removeTeamMember = async (id, requesterId) => {
    if (id === requesterId) {
